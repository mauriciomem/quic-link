...
(`serve` for `agent`, `connect` for `daemon --server NAME`); the full,
current list is `docs/cli.md`'s verb table, which `main.go`'s own comment
points at. It is deliberately thin — the accept-loop and protocol logic it
