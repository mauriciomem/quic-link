...
// Under pinning (ADR-0004) it is the sole trust credential; the same value the
// peer's keygen printed and the agent's authorized_clients pins are compared
// against. Empty Pin = unauthenticated.
...
func IdentityFromCerts(certs []*x509.Certificate) (Identity, error) {
	...
	// Single source of truth for the pin formula (INV): identity.PinFromCert.
